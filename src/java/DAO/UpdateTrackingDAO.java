...
public class UpdateTrackingDAO extends Utils.ConnectJDBC {
    ...
    public boolean updateTracking(UpdateTracking c) {
        boolean check = false;
        String sql = "UPDATE `studentmanagement`.`update_tracking` SET `Update_note`=?,`Update_date`=now() WHERE (`update_id`=?);";
        try {
    ...
    public boolean addTracking(UpdateTracking c) {
        boolean check = false;
        String sql = "INSERT INTO `studentmanagement`.`update_tracking` (`milestone_id`,`tracking_id`,`Update_note`)"
                + "VALUES (?,?,?);";
        try {
            PreparedStatement pre = conn.prepareStatement(sql);
    ...
    public UpdateTracking checkTracking(int milestone_id, int tracking_id) {
        ...
        String sql = "select * from `studentmanagement`.`update_tracking` where milestone_id =" + milestone_id + " and tracking_id =" + tracking_id;
    ...
    public UpdateTracking getUpdateTracking(int update_id) {
        UpdateTracking tracking = null;
        String sql = "select * from `studentmanagement`.`update_tracking` where update_id=" + update_id;
        ResultSet rs = getData(sql);
        try {
            while (rs.next()) {
    ...
    public List<UpdateTracking> getList(Integer team, Integer milestone, Integer tracking, Integer feature, int start, int limit, User user, String sort, boolean statusSort) {
        ...
        String sql = "CALL `studentmanagement`.search('\\'%%\\'','studentmanagement','update_tracking','" + (milestone != null ? " and milestone_id=" + milestone : "") + (tracking != null ? " and tracking_id=" + tracking : "")
                ...
                + (user.getRole_id() == 1 ? "" : user.getRole_id() == 2 ? " and milestone_id in (select milestone_id from `studentmanagement`.`milestone`,`studentmanagement`.`class` where milestone.class_id = class.class_id and trainer_id=" + user.getUser_id() + ")" : (user.getRole_id() == 4 ? " and milestone_id in (select milestone_id from `studentmanagement`.`milestone`,`studentmanagement`.`class_user` where class_user.class_id=milestone.class_id and user_id=" + user.getUser_id() + ")" : " and milestone_id in (select milestone_id from `studentmanagement`.`milestone`,`studentmanagement`.`class`,`studentmanagement`.`subject` where class.class_id=milestone.class_id and class.subject_id=subject.subject_id and author_id=" + user.getUser_id() + ")"))
