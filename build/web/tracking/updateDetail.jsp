...
<html>
    ...
    <c:set var="trackingChoose" value="${requestScope.TRACKING_UPDATE_CHOOSE}"></c:set>
    <c:set var="classChoose" value="${sessionScope.CLASS_CHOOSE}"></c:set>
        <head>
        <jsp:include page="../included/head.jsp" />
        <link href="css/styles.css" rel="stylesheet" />
        <title>Update Tracking Detail · ${trackingChoose!=null?(trackingChoose.getTracking().getFunction().getFunction_name().concat("_").concat(trackingChoose.getTracking().getAssignee().getFull_name()).concat("_").concat(trackingChoose.getMilestone().getMilestone_name())):"Add"}</title>
    </head>
    <body>
        <div class="nav-fixed">
                                ...
                                <div class="page-header-content pt-4">
                                    <div class="d-flex justify-content-between align-items-sm-center flex-column flex-sm-row mb-4">
                                        <div class="me-4 mb-3 mb-sm-0">
                                            <h1 class="mb-0 text-bold">Update Tracking Detail · ${trackingChoose!=null?(trackingChoose.getTracking().getFunction().getFunction_name().concat("_").concat(trackingChoose.getTracking().getAssignee().getFull_name()).concat("_").concat(trackingChoose.getMilestone().getMilestone_name())):"Add"}</h1>
                                    ...
                                    <nav class="mt-4 rounded bg-light" aria-label="breadcrumb">
                                        <ol class="breadcrumb px-3 py-2 rounded mb-0">
                                            ...
                                            <li class="breadcrumb-item"><a href="tracking?id=${requestScope.TRACKING_CHOOSE.getTracking_id()}">${trackingChoose!=null?trackingChoose.getTracking().getFunction().getFunction_name().concat("_").concat(trackingChoose.getTracking().getAssignee().getFull_name()):"_"}</a></li>
                                            <li class="breadcrumb-item active"><a href="updateTracking?tracking=${requestScope.TRACKING_CHOOSE.getTracking_id()}">Update Tracking List</a></li>
                                            <li class="breadcrumb-item active">${trackingChoose!=null?(trackingChoose.getTracking().getFunction().getFunction_name().concat("_").concat(trackingChoose.getTracking().getAssignee().getFull_name()).concat("_").concat(trackingChoose.getMilestone().getMilestone_name())):"Add"}</li>
                                        </ol>
                                    </nav>
                                </div>
                            </div>
                            <div class="card card-waves p-4 mb-4 mt-5">
                                <div class="row align-items-end">
                                    <div class="mb-3 col-xl-3 col-md-6">
                                        ...
                                        <c:choose>
                                            <c:when test="${trackingChoose != null}" >
                                                <input type="text" required=""  class="form-control border-0 border-bottom ${loginUser.getRole_id()<4?'bg-primary-soft':'bg-transparent'}" value="${trackingChoose.getTracking().getClassroom().getClass_code()}" disabled="">
                                                <input type="text" name="class" id="class" class="form-control" value="${trackingChoose.getTracking().getClassroom().getClass_id()}" hidden="">
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <div class="mb-3 col-xl-3 col-md-6">
                                        ...
                                        <c:choose>
                                            <c:when test="${trackingChoose != null}" >
                                                <input type="text" required=""  class="form-control border-0 border-bottom ${loginUser.getRole_id()<4?'bg-primary-soft':'bg-transparent'}" value="${trackingChoose.getTracking().getTeam().getTeam_name()}" disabled="">
                                                <input type="text" name="team" id="team" class="form-control" value="${trackingChoose.getTracking().getTeam().getTeam_id()}" hidden="">
                                            </c:when>
                                            <c:otherwise>
                                                <form action="updateTracking" method="POST">
                                                    <input type="hidden" name="service" value="add"/>
                                    ...
                                    <div class="mb-3 col-xl-6 col-md-12">
                                        ...
                                        <c:choose>
                                            <c:when test="${trackingChoose != null}" >
                                                <input type="text" required=""  class="form-control border-0 border-bottom ${loginUser.getRole_id()<4?'bg-primary-soft':'bg-transparent'}" value="${trackingChoose.getTracking().getFeature().getFeature_name()}" disabled="">
                                                <input type="text" name="feature" id="feature" class="form-control" value="${trackingChoose.getTracking().getFeature().getFeature_id()}" hidden="">
                                            </c:when>
                                            <c:otherwise>
                                                <form action="updateTracking" method="POST">
                                                    <input type="hidden" name="service" value="add"/>
                                ...
                                <c:if test="${((trackingChoose==null&&requestScope.LIST_TRACKING.size()>0&&requestScope.LIST_MILESTONE.size()>0)||trackingChoose!=null)}">
                                    <form action="updateTracking" method="POST">
                                        <div class="row">
                                            <div class="mb-3 col-xl-6 col-md-6">
                                                ...
                                                <c:choose>
                                                    <c:when test="${trackingChoose != null}" >
                                                        <input type="hidden" name="class" value="${trackingChoose.getTracking().getClassroom().getClass_id()}"/>
                                                        <input type="hidden" name="team" value="${trackingChoose.getTracking().getTeam().getTeam_id()}"/>
                                                        <input type="hidden" name="feature" value="${trackingChoose.getTracking().getFeature().getFeature_id()}"/>
                                                        <input type="hidden" name="id" value="${trackingChoose.getUpdate_id()}"/>
                                                        <input type="text" required=""  class="form-control border-0 border-bottom ${loginUser.getRole_id()<4?'bg-primary-soft':'bg-transparent'}" value="${trackingChoose.getTracking().getFunction().getFunction_name()}_${trackingChoose.getTracking().getAssignee().getFull_name()}" disabled="">
                                                        <input type="text" name="tracking" id="tracking" class="form-control" value="${trackingChoose.getTracking_id()}" hidden="">
                                                    </c:when>
                                                    <c:otherwise>
                                                        <input type="hidden" name="class" value="${sessionScope.CLASS_CHOOSE!=null?sessionScope.CLASS_CHOOSE.getClass_id():sessionScope.LIST_CLASS.get(0).getClass_id()}"/>
                                                        ...
                                                        <select ${loginUser.getRole_id()==4?"":"disabled=''"} ${tracking!=null&&tracking.getAssignee_id()!=loginUser.getUser_id()?"disabled=''":""} id="tracking" required="" ${requestScope.LIST_TRACKING==null?'disabled=""':''} name="tracking" class="form-control col border-0 border-bottom ${loginUser.getRole_id()<4?'bg-primary-soft':'bg-transparent'}" >
                                                            <c:forEach items="${requestScope.LIST_TRACKING}" var="tracking">
                                                                <c:if test="${tracking.getAssignee_id()==loginUser.getUser_id()}">
                                                                    <c:choose>
                                                                        <c:when test="${requestScope.TRACKING_CHOOSE!=null&&requestScope.TRACKING_CHOOSE.getTracking_id()==tracking.getTracking_id()}">
                                                                            <option class="text-capitalize" selected=""  value="${tracking.getTracking_id()}">${tracking.getFunction().getFunction_name()}</option>
                                                                        </c:when>
                                                                        <c:otherwise>
                                                                            <option class="text-capitalize" value="${tracking.getTracking_id()}">${tracking.getFunction().getFunction_name()}</option>
                                                                        </c:otherwise>
                                                                    </c:choose>
                                                                </c:if>
                                                            </c:forEach>          
                                        ...
                                        <div class="mb-3 form-floating">
                                            <textarea ${loginUser.getRole_id()==4?"":"disabled=''"} class="form-control ${loginUser.getRole_id()<4?'bg-primary-soft':'bg-transparent'}" placeholder="User Note" id="trackingNote" name="note">${trackingChoose!=null?trackingChoose.getUpdate_note():''}</textarea>
